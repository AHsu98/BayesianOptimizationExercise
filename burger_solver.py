...
def get_burger_solver_periodic(alpha,kappa,k = 1e-4,n = 1000):
    ...
    def solve_burgers(u0,final_t = 1.):
        num_timesteps = int(final_t/k)
        tvals = np.arange(0,final_t,k)
        u_vals = np.zeros((int(num_timesteps),n))
        u_vals[0] = u0
        u = u0
        for i in range(int(num_timesteps)):
            u = burger_split_step(u)
            u_vals[i] = u
        return RectBivariateSpline(tvals,xgrid,u_vals),tvals
